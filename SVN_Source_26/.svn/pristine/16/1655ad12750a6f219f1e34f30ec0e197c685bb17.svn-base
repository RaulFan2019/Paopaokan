 ...
 * 配速的曲线
 */
public class FriendHistoryPaceChartView extends DemoView {
    ...
    /* Constructor */
    public FriendHistoryPaceChartView(Context context) {
        super(context);
    }

    public FriendHistoryPaceChartView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public FriendHistoryPaceChartView(Context context, AttributeSet attrs, int defStyle) {
     ...
     * @param color
     */
    public void addLine(List<HistoryLocEntity> mPlayList, int color, boolean draw) {
        ...
        for (int i = 0; i < mPlayList.size(); i += drawStep) {
            HistoryLocEntity location = mPlayList.get(i);
            double x = i;
