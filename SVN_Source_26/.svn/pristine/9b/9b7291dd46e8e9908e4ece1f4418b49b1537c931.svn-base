...
public class FolderAdapter extends BaseAdapter {
	...
	public View getView(int i, View view, ViewGroup viewGroup) {
		ViewHolder holder;
		if (view == null) {
			view = mInflater.inflate(R.layout.item_folder_list, viewGroup, false);
		...
		if (holder != null) {
			if (i == 0) {
				...
				if (mFolders.size() > 0) {
					...
					Picasso.with(mContext).load(new File(f.cover.path)).error(R.drawable.iv_default_error)
	...
	class ViewHolder {
		...
		void bindData(Folder data) {
			...
			// 显示图片
			Picasso.with(mContext).load(new File(data.cover.path)).placeholder(R.drawable.iv_default_error)
