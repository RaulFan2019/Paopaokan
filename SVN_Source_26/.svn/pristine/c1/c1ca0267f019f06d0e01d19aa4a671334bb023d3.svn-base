...
import com.app.pao.adapter.GroupMemberRankingTotalAdapter;
 ...
 */
public class GroupMemberRankingTotalFragment extends BaseFragment implements View.OnClickListener{
    ...
    @ViewInject(R.id.rv_member_ranking_total_list)
    private RecyclerView mTotalRankingRv;
    ...
    private GroupMemberRankingTotalAdapter mTotalAdapter;
    private List<GetGroupMemberSortResult> mTotalList;
    ...
    public static GroupMemberRankingTotalFragment newInstance() {
        GroupMemberRankingTotalFragment nameFragment = new GroupMemberRankingTotalFragment();
        return nameFragment;
    }
    ...
    protected int getLayoutId() {
        return R.layout.fragment_member_ranking_total;
    }
...
//        mTotalRankingRv.setLayoutManager(new FullyLinearLayoutManager(mContext));
        mTotalRankingRv.setLayoutManager(new LinearLayoutManager(mContext));
    }

    private void refreshView() {
        //    if (mTotalAdapter == null) {
        mTotalAdapter = new GroupMemberRankingTotalAdapter(mContext,mGroupInfo, mTotalList);
        mTotalRankingRv.setAdapter(mTotalAdapter);
        //      } else {
        ...
        for (int i = 0; i < mTotalList.size(); i++) {
            if (mTotalList.get(i).getId() == LocalApplication.getInstance().getLoginUser(mContext).getUserId()) {
                mRanking = i + 1;
                break;
            }
        }
        mParentFragment.mRankTotal.setText(mTotalList.size()+"");
        mParentFragment.mRank.setText(mRanking+"");
//        Spanned showText = Html.fromHtml("<font color=\"#888888\">您在" + mTotalList.size() + "名成员中排名第</font><font color=\"#F06522\" ><big> " + mRanking + " </big></font>名");
//        mTotalRankingTextTv.setText(showText);
    }
    ...
    private void postGetSortWeekRequest() {
        ...
        http.send(HttpRequest.HttpMethod.POST, POST_URL, params, new MyRequestCallBack(mContext) {
            ...
            protected void onRightResponse(String Response) {
                ...
                mTotalList.addAll(JSON.parseArray(Response, GetGroupMemberSortResult.class));
                if (mPostHandler != null) {
                    Message msg = new Message();
