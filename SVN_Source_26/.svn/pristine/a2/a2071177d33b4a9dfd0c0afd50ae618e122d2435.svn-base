...
public class HttpExceptionHelper {
    ...
    public static String getErrorMsg(final Throwable throwable) {
        String ToastStr = "";
        int errorCode = 0;
        if (throwable instanceof org.xutils.ex.HttpException) { // 网络错误
            org.xutils.ex.HttpException httpEx = (org.xutils.ex.HttpException) throwable;
    ...
    public static String getErrorMsg(HttpException Exception) {
        int errorCode = Exception.getExceptionCode();
        String ToastStr = "请检查你的网络";
        String tagStr = "";
        switch (errorCode) {
            case 400:
                ToastStr = 400 + ":服务器错误";
                tagStr = 400 + "服务器不理解请求的语法";
                break;
            case 401:
                ToastStr = 401 + ":服务器授权错误";
                tagStr = 401 + "请求要求身份验证";
            ...
            default:
                ToastStr = "请检查你的网络";
                tagStr = errorCode + Exception.getMessage();
                break;
        }
        return ToastStr;
    }
     ...
     */
    public static void show(Context context, HttpException Exception, String msg) {
        int errorCode = Exception.getExceptionCode();
        String ToastStr = "";
        String tagStr = "";
        switch (errorCode) {
            ...
            case 500:
                ...
                break;
            default:
                ToastStr = errorCode + Exception.getMessage();
                tagStr = errorCode + Exception.getMessage();
                break;
        }
