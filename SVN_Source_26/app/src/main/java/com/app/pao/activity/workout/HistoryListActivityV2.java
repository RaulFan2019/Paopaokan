...
/**
 * Created by Raul.Fan on 2016/5/24.
 */
@ContentView(R.layout.activity_history_list)
public class HistoryListActivityV2 extends BaseAppCompActivity implements View.OnClickListener {

    /* contains */
    private static final String TAG = "HistoryListActivityV2";
    ...
    /* local data */
    private List<GetWorkoutListResult.WorkoutEntity> mWorkoutEntityList = new ArrayList<>();//历史列表(网络)
    private List<HistoryListWithTitleItem> mHistoryList = new ArrayList<>();//历史列表（显示）
    private List<String> nameList = new ArrayList<>();//用于记录还没来得及上传的跑步记录的名字

    private HistoryListAdapterV2 mAdapter;//适配器
    private String lastItemMonth = "";//最后一个月份字符串
    private boolean postAble = true;//是否可以POST
    private int mHistoryCount = 0;//跑步历史数量
    private boolean mHasMoreHistory = true;
    private int eyePosition;//眼睛的位置
    private boolean mLoadLocalData = false;
            ...
            if (requestCode == RESULT_BACK_DELECT) {
                if (data.hasExtra("isDelete")) {
                    ...
                    if (isDelete) {
                        ...
                        int pos = 0;
                        for (; pos < mHistoryList.size(); pos++) {
                            if (mHistoryList.get(pos).entity != null
                                    && mHistoryList.get(pos).entity.starttime.equals(startTime)) {
                                break;
                            }
                        }
    ...
    protected void initViews() {
        ...
        mAdapter.setOnItemClickListener(new HistoryListAdapterV2.OnItemClickListener() {
            ...
            public void setOnItemClickListener(int position) {
                ...
                bundle.putString("workoutname", workoutEntity.name);
                bundle.putInt("workoutid", workoutEntity.id);
                startActivityForResult(HistoryInfoActivityV2.class, bundle, RESULT_BACK_DELECT);
            }
        });
    ...
    private void doLoadMore() {
        ...
        if (postAble) {
            if (mHasMoreHistory) {
                ...
                T.showShort(mContext, "没有更多跑步记录了");
                mFreshLayout.loadmoreFinish(PullToRefreshLayout.DONE);
            }
        }
    }
     ...
     * 获取游客的历史数据
     */
    private void touristRefreshByLoc() {
        List<DBEntityWorkout> workoutList = WorkoutData.getWorkOutList(mContext, AppEnum.DEFAULT_USER_ID);
        if (workoutList != null && workoutList.size() != 0) {
            for (DBEntityWorkout workout : workoutList) {
                GetWorkoutListResult.WorkoutEntity workoutEntity = new GetWorkoutListResult.WorkoutEntity();
                workoutEntity.name = workout.name;
                workoutEntity.duration = workout.duration;
                workoutEntity.length = workout.length;
                mWorkoutEntityList.add(workoutEntity);
            }
        }
        parseWorkoutList(mWorkoutEntityList);
        mAdapter.notifyDataSetChanged();
        mLoadViewRl.setVisibility(View.GONE);
    }
     ...
     * 获取未上传的跑步记录
     */
    private void getUnLoadWorkout() {
        mWorkoutEntityList.clear();
        int userId = LocalApplication.getInstance().getLoginUser(mContext).getUserId();
        nameList = UploadData.getUnfinishWorkoutNameList(mContext, userId);
        if (nameList.size() != 0) {
            for (int j = 0; j < nameList.size(); j++) {
                DBEntityWorkout dbEntityWorkout = WorkoutData.getWorkoutByName(mContext, nameList.get(j));
                if (dbEntityWorkout != null) {
                    GetWorkoutListResult.WorkoutEntity workoutEntity = new GetWorkoutListResult.WorkoutEntity();
                    workoutEntity.name = dbEntityWorkout.name;
                    workoutEntity.duration = dbEntityWorkout.duration;
                    workoutEntity.length = dbEntityWorkout.length;
                    workoutEntity.type = 0;
                    mWorkoutEntityList.add(workoutEntity);
                }
            }
        }
     ...
     * 当没有网络的时候获取本地数据
     */
    private void getLocalWorkout() {
        mWorkoutEntityList.clear();
        mLoadLocalData = true;
        List<DBEntityWorkout> workoutList = WorkoutData.getWorkOutList(mContext,
        ...
        if (workoutList != null && workoutList.size() != 0) {
            for (DBEntityWorkout workout : workoutList) {
                ...
                workoutEntity.duration = workout.duration;
                workoutEntity.length = workout.length;
                if (nameList.contains(workout.name)) {
                    workoutEntity.type = 0;
                } else {
                    workoutEntity.type = 1;
                }
                mWorkoutEntityList.add(workoutEntity);
            }
        }
