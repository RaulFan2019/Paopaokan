 ...
 * 跑步回放的曲线
 */
public class FriendHistoryHeartrateChartView extends DemoView {
    ...
    /* Constructor */
    public FriendHistoryHeartrateChartView(Context context) {
        super(context);
    }

    public FriendHistoryHeartrateChartView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public FriendHistoryHeartrateChartView(Context context, AttributeSet attrs, int defStyle) {
     ...
     * @param color
     */
    public void addLine(List<HistoryHeartrateEntity> mPlayList, int color, boolean draw) {
        ...
        for (int i = 0; i < mPlayList.size(); i += drawStep) {
            HistoryHeartrateEntity entityHeartrate = mPlayList.get(i);
            double x = i;
            linePoint.add(new PointD(x, 0));
            mLabels.add("");
            if (entityHeartrate.bmp == 0) {
                heartratePoint.add(0.0);
                continue;
            }
            if (maxAx < entityHeartrate.bmp) {
                maxAx = entityHeartrate.bmp;
            }
            double y = entityHeartrate.bmp;
