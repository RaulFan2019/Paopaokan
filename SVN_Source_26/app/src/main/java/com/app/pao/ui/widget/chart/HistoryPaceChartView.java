 ...
 * 配速的曲线
 */
public class HistoryPaceChartView extends DemoView {
    ...
    /* Constructor */
    public HistoryPaceChartView(Context context) {
        super(context);
    }

    public HistoryPaceChartView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public HistoryPaceChartView(Context context, AttributeSet attrs, int defStyle) {
     ...
     * @param color
     */
    public void addLine(List<DBEntityLocation> mPlayList, int color, boolean draw) {
        ...
        for (int i = 0; i < mPlayList.size(); i += drawStep) {
            DBEntityLocation location = mPlayList.get(i);
            double x = i;
