 ...
 * 跑步回放的曲线
 */
public class HistoryHeartrateChartView extends DemoView {
    ...
    /* Constructor */
    public HistoryHeartrateChartView(Context context) {
        super(context);
    }

    public HistoryHeartrateChartView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public HistoryHeartrateChartView(Context context, AttributeSet attrs, int defStyle) {
     ...
     * @param color
     */
    public void addLine(List<DBEntityHeartrate> mPlayList, int color, boolean draw) {
        ...
        for (int i = 0; i < mPlayList.size(); i += drawStep) {
            DBEntityHeartrate entityHeartrate = mPlayList.get(i);
            double x = i;
