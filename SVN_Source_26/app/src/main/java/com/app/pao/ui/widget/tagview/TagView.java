...
public class TagView extends RelativeLayout {
	...
	private void initialize(Context ctx, AttributeSet attrs, int defStyle) {
		...
		this.layoutSingleLine = typeArray.getBoolean(R.styleable.TagView_layoutSingleLine, false);
		typeArray.recycle();
	}
	...
	private void drawTags() {
		...
		for (Tag item : mTags) {
			...
			TextView tagPlusTv = (TextView) tagLayout.findViewById(R.id.tv_tag_item_contain_plus);
			tagPlusTv.setText(tag.textPlus);
			tagPlusTv.setTextColor(tag.tagPlusTextColor);
			tagPlusTv.setTextSize(TypedValue.COMPLEX_UNIT_SP,tag.tagPlusTextSize);

			// calculate　of tag layout width
			float tagWidth = tagView.getPaint().measureText(tag.text.toString() + tag.textPlus.toString()) + textPaddingLeft + textPaddingRight;
...
//						TagView.this.remove(position);
						if (mDeleteListener != null&& isEnabled()) {
			...
			ImageView addImg = (ImageView) tagLayout.findViewById(R.id.iv_tag_item_add);
			if(tag.isShowImg) {
				addImg.setVisibility(VISIBLE);
				addImg.setImageResource(tag.imgDrawableRes);
				int offset = Utils.dpToPx(getContext(), 2f);
				addImg.setPadding(offset, textPaddingTop, textPaddingRight + offset, texPaddingBottom);
				tagWidth += addImg.getDrawable().getIntrinsicWidth() + textPaddingLeft + textPaddingRight;
			}else {
...
//			gd_normal.setStroke(Utils.dpToPx(getContext(), tag.layoutBorderSize), tag.layoutBorderColor);
			gd_normal.setStroke(1, tag.layoutBorderColor);
		}
	 ...
	 * @return
	 */
	public void refreshOneTag(int position,Tag tag){
		Tag cTag = mTags.get(position);
		cTag = tag;
		drawTags();
	}
